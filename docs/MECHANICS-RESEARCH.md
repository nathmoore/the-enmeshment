...
**2. Verbatim reproduction from a knowledge file is unreliable — and we don't want it.** Models strongly prefer to *paraphrase* knowledge rather than quote it, and copyright-sensitivity filters can even cause refusals to reproduce text. You cannot force exact reproduction with confidence, even for a GPT's own uploaded content. For this game that is a non-problem: the player **wants** a natural, in-voice verdict, not a recited file. The robust design is therefore to have the GPT *generate the verdict in the Directorate voice* within a fixed structure — precisely the template approach already in [output-templates.txt](../src/knowledge/output-templates.txt) (paragraphs the agent voices, not a block it must echo). The thing that **must** be exact — the dossier-page link — is handled separately (finding 3). *Refs:* OpenAI Developer Community, "When will a custom GPT quote from a doc in its knowledge source?" (community.openai.com/t/when-will-a-custom-gpt-quote-from-a-doc-in-its-knowledge-source/613128); OpenAI, "Knowledge in GPTs" (help.openai.com/en/articles/8843948-knowledge-in-gpts).

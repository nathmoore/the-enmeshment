...
> **Runtime caveat (see "Platform capabilities & constraints" below):** the "internal confidence accumulation" here is *emergent reasoning over the visible transcript*, **not a stored variable** the GPT increments — the runtime keeps no state between turns. It behaves *as if* tracking confidence only when the instructions pin it down: a minimum question count, fixed **anchor questions**, and an explicit **stop rule** (ties to §IV and the Epic 1 elicitation spec). Without those, "reveal when ready" drifts — early or wandering classifications.
...
### Platform capabilities & constraints — what the GPT runtime can and can't do (June 2026)

> *Added June 2026. Grounds the **archetype-resolution mechanics** in what a Custom GPT can actually execute at runtime — distinct from §III's store/safety-facing platform notes. The question that prompted this: can the GPT track a confidence level, run an adaptive multi-question flow, return a dossier verbatim, and/or hand the player a link to a hosted dossier? Short version: the runtime adds **no special machinery** — it's a stateless LLM following instructions — so each capability is really a prompting-discipline question. The four findings below set the constraints the elicitation spec (§IV) and the content architecture must design within.*

**1. State and "confidence" are simulated, not real.** A Custom GPT has no persistent variables and no hidden scratchpad — it is a stateless model whose only working memory is the visible conversation transcript. There is no counter it increments toward a threshold. The "internal confidence accumulation → reveal" model (above) works, but only as *emergent reasoning re-derived each turn from the transcript*. It behaves reliably **as if** tracking confidence when the instructions make the behaviour explicit: a pinned **minimum question count**, a fixed backbone of **anchor questions**, and a written **stop rule** for when to classify. Multi-step flows of this size are well within reach for current models, but consistency is a *prompting* property, not a runtime guarantee — without the scaffold the GPT classifies too early or wanders. This is exactly what the Epic 1 elicitation spec and §IV's playbook already specify; the runtime finding is that those scaffolds are **load-bearing**, not optional polish. *Refs:* OpenAI, "Key Guidelines for Writing Instructions for Custom GPTs" (help.openai.com/en/articles/9358033-key-guidelines-for-writing-instructions-for-custom-gpts); OpenAI Cookbook, GPT-5 prompting guide — multi-step reliability and self-check before finalising (cookbook.openai.com/examples/gpt-5).

**2. Verbatim reproduction from a knowledge file is unreliable — and we don't want it.** Models strongly prefer to *paraphrase* knowledge rather than quote it, and copyright-sensitivity filters can even cause refusals to reproduce text. You cannot force exact reproduction with confidence, even for a GPT's own uploaded content. For this game that is a non-problem: the player **wants** a natural, in-voice verdict, not a recited file. The robust design is therefore to have the GPT *generate the verdict in the Directorate voice* within a fixed structure — precisely the template approach now in the `OUTPUT FORMAT` section of [instructions.md](../src/instructions.md) (paragraphs the agent voices, not a block it must echo; folded in from the retired `output-templates.txt`, 2026-06-13). The thing that **must** be exact — the dossier-page link — is handled separately (finding 3). *Refs:* OpenAI Developer Community, "When will a custom GPT quote from a doc in its knowledge source?" (community.openai.com/t/when-will-a-custom-gpt-quote-from-a-doc-in-its-knowledge-source/613128); OpenAI, "Knowledge in GPTs" (help.openai.com/en/articles/8843948-knowledge-in-gpts).

**3. Emitting an exact link needs no browsing or Action — but the URL must be looked up, not guessed.** A URL is just text in the reply; the GPT does **not** need the Web Browsing capability or a GPT Action to *output* a link (those are only needed to *read* a page's contents at runtime, which this design never requires — the dossier content lives in knowledge). Reliability hinges on one rule: the agent must **copy** the archetype's URL from an explicit archetype→URL map held in a knowledge file, never **construct** it from a slug pattern. Given a lookup table it copies the right link near-deterministically; asked to infer `…/files/<slug>` itself, it will occasionally mangle the slug. This makes the user's chosen delivery model — paraphrased verdict + exact dossier link — cheap and dependable, with no Actions/OpenAPI machinery to build or maintain. *Refs:* OpenAI, GPT Actions — introduction (developers.openai.com/api/docs/actions/introduction); OpenAI Cookbook, GPT Actions library — GitHub example (cookbook.openai.com/examples/chatgpt/gpt_actions_library/gpt_action_github); OpenAI, "Creating and editing GPTs" — per-GPT Web Browsing capability (help.openai.com/en/articles/8554397-creating-and-editing-gpts).

**4. Knowledge files are uploaded static snapshots — they cannot point at a live GitHub file.** There is no feature to "link" a knowledge file to a live URL; knowledge is *uploaded* files, indexed for retrieval (RAG). Limits: up to **20 files**, **≤512 MB** and **≤2 M tokens** each; combined knowledge can exceed the ~32k context window because files are **chunked and retrieved**, not loaded whole, and a single document beyond ~110k tokens is **truncated** to its first ~110k. To use *live* content you would need either the Web Browsing capability or a GPT Action fetching `raw.githubusercontent.com` — both add latency, can fail mid-session, and pull raw text into a limited context (risking truncation/budget blow-out) with no pre-indexing. For The Enmeshment's small, slowly-changing canon the trade is clear: **keep uploaded `.txt`**; the only cost is the manual **re-upload on release** already documented in the ARCHITECTURE.md deployment process. Live-fetch would only earn its keep if content changed often or grew too large to upload — neither applies. *Refs:* OpenAI, "Knowledge in GPTs" (help.openai.com/en/articles/8843948-knowledge-in-gpts); OpenAI Developer Community, "Workaround with knowledge files bigger than context window" (community.openai.com/t/workaround-with-knowledge-files-bigger-than-context-window/848792); OpenAI Developer Community, "What is the knowledge file limit that can be uploaded to train custom GPTs" (community.openai.com/t/what-is-the-knowledge-file-limit-that-can-be-uploaded-to-train-custom-gpts/653244).

**Design takeaways for archetype resolution.** (a) Confidence/stop logic must be *written into instructions* (anchor questions + minimum count + stop rule), not assumed of the runtime. (b) Author the verdict as a *voiced template*, not a verbatim block. (c) Deliver the result as a paraphrased verdict **+ an exact link copied from an archetype→URL map knowledge file** — no Actions needed. (d) Keep canon as uploaded `.txt`; accept re-upload-on-release as the cost of runtime reliability. Items (b)–(d) feed directly into the Epic 1 content-architecture deliverable.

*Sources: OpenAI Help Center (custom-GPT instructions, knowledge, editing/capabilities); OpenAI Developer Platform (GPT Actions); OpenAI Cookbook (GPT-5 prompting guide; GPT Actions library); OpenAI Developer Community threads on verbatim quoting and knowledge-file limits. Full URLs in the Bibliography.*
...
**GPT platform capabilities & constraints (runtime — §I)**
- OpenAI Help Center. *Key Guidelines for Writing Instructions for Custom GPTs.* [link](https://help.openai.com/en/articles/9358033-key-guidelines-for-writing-instructions-for-custom-gpts)
- OpenAI Help Center. *Knowledge in GPTs* (uploaded files, retrieval, limits). [link](https://help.openai.com/en/articles/8843948-knowledge-in-gpts)
- OpenAI Help Center. *Creating and editing GPTs* (per-GPT capabilities incl. Web Browsing). [link](https://help.openai.com/en/articles/8554397-creating-and-editing-gpts)
- OpenAI Developer Platform. *GPT Actions — Introduction.* [link](https://developers.openai.com/api/docs/actions/introduction)
- OpenAI Cookbook. *GPT-5 prompting guide* (multi-step reliability, self-check). [link](https://cookbook.openai.com/examples/gpt-5)
- OpenAI Cookbook. *GPT Actions library — GitHub example.* [link](https://cookbook.openai.com/examples/chatgpt/gpt_actions_library/gpt_action_github)
- OpenAI Developer Community. *When will a custom GPT quote from a doc in its knowledge source?* [link](https://community.openai.com/t/when-will-a-custom-gpt-quote-from-a-doc-in-its-knowledge-source/613128)
- OpenAI Developer Community. *Workaround with knowledge files bigger than context window.* [link](https://community.openai.com/t/workaround-with-knowledge-files-bigger-than-context-window/848792)
- OpenAI Developer Community. *What is the knowledge file limit that can be uploaded to train custom GPTs.* [link](https://community.openai.com/t/what-is-the-knowledge-file-limit-that-can-be-uploaded-to-train-custom-gpts/653244)

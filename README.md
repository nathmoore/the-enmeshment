...
This project's objective is to be completely enjoyable on one level — a fun, shareable, elevated-BuzzFeed-quiz experience — while pointing toward deeper thought on another:
...
## How it ships (who deploys what)

The repo is the source of truth; the live GPT is a **deployment target**, and only the
maintainer can push to it. Concretely:

- **The GPT** — [`src/instructions.md`](src/instructions.md) (the fenced block only) is pasted
  into the GPT Builder by hand, and the five [`src/knowledge/*.txt`](src/knowledge/) files are
  re-uploaded, on each release. Releases are tagged `gpt-vX.Y.Z`. **A merged PR that touches
  `src/` is not live until the maintainer redeploys** — the store version can lag `main`.
- **The site** — [`site/`](site/) builds and deploys to GitHub Pages automatically on every
  push to `main` ([`pages.yml`](.github/workflows/pages.yml)). The internal `docs/` are never
  published there; they're only readable here.
- **The docs** — live the moment they're merged. [`DECISIONS.md`](docs/planning/DECISIONS.md)
  is the record; check it before reopening a settled call.
